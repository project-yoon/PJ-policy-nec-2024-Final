...
function issueTreeSigungu() {
    ...
    btnCities.forEach((btnCity, btnCityidx) => {
        ...
        btnCity.addEventListener('click', (e) => {
                console.log('(z) (z) (z) keyCode::' + e.keyCode);
                console.log('(z) (z) (z)ENTERtargetID::' + e.target.id);
                console.log('(2zzz2zzzz22) enter ok');
                titleChange(btnCities, btnCity);
                console.log('(3zzzz3zzzz33) title chg');
                sigunguMap.setAttribute('data-open', 'open');
                console.log('(4zzz44zzzzzzz4) open');
                sigunguMap.setAttribute('data-inmap', btnCity.getAttribute('data-city'));
                for (let i in sigunguList) {
                    console.log('(5zzzz5zzzz55) sigunguList');
                    if (sigunguMap.getAttribute('data-inmap') == i) {
                        sigunguLineup.innerHTML = sigunguList[i];
                        console.log('(6zzzz6zzzz66) innerHTML');
                    }
                }
                // 첫번째 시군구로 포커스 이동
                setTimeout(() => {
                    console.log('(7z7zzzz77) focus go');
                    sigunguLineup.querySelector('li:first-child button').focus();
                    console.log('----------------------------포커스이동완료--------------------');
                }, 1);
                // 시군구 키워드 숨김
                console.log('(8zzzz88zzzzz8) hdd');
                topParents.setAttribute('data-keyword', '');
                console.log('(9zzzz9zzzz99) remove');
                ChangeSigunguButton();
                console.log('(10zzzzzz1010) zz');
            titleChange(btnCities, btnCity);
        ...
        btnCity.addEventListener('keydown', (e) => {
            ...
            if (e.which === 13) {
                console.log('(ㅠㅠㅠㅠ2) (2) (2) keyCode::' + e.keyCode);
                console.log('(ㅠㅠㅠㅠ2) (2) (2)ENTERtargetID::' + e.target.id);
                console.log('(ㅠㅠㅠㅠㅠㅠ2222) enter ok');
                titleChange(btnCities, btnCity);
                console.log('(ㅠㅠㅠㅠㅠ3333) title chg');
                sigunguMap.setAttribute('data-open', 'open');
                console.log('(ㅠㅠㅠㅠ4444) open');
                sigunguMap.setAttribute('data-inmap', btnCity.getAttribute('data-city'));
                for (let i in sigunguList) {
                    console.log('(ㅠㅠㅠㅠ5555) sigunguList');
                    if (sigunguMap.getAttribute('data-inmap') == i) {
                        sigunguLineup.innerHTML = sigunguList[i];
                        console.log('(ㅠㅠㅠㅠㅠ6666) innerHTML');
                    }
                }
                // 첫번째 시군구로 포커스 이동
                setTimeout(() => {
                    console.log('(ㅠㅠㅠㅠㅠ7777) focus go');
                    sigunguLineup.querySelector('li:first-child button').focus();
                }, 1);
                // 시군구 키워드 숨김
                console.log('(ㅠㅠㅠㅠ8888) hdd');
                topParents.setAttribute('data-keyword', '');
                console.log('(ㅠㅠㅠ9999) remove');
                ChangeSigunguButton();
                console.log('(ㅠㅠㅠㅠㅠ101010) zz');
            }
