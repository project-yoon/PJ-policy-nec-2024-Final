...
function issueTree() {
    ...
    mapLists.forEach((mapList, index) => {
        ...
        mapList.addEventListener('keydown', (e) => {
            ...
            if (e.keyCode === 'Enter' || e.key === ' ') {
                console.log('(2-1) enter ok');
                // title="선택됨" 적용
                titleChange(mapLists, mapList);
                console.log('(3-1) title chg');
                // 시도 키워드 노출
                topParents.setAttribute('data-keyword', '5');
                console.log('(4-1) open');
                // 타이틀 변경
                for (let i in keywordCol05Title) {
                    console.log('(5-1) sigunguList');
                    if (mapList.getAttribute('data-area') == i) {
                        mapTitle.innerHTML = keywordCol05Title[i];
                        console.log('(6-1) innerHTML');
                    }
                }
                // 키워드 변경
                for (let i in keywordCol05Contents) {
    ...
    btnCities.forEach((btnCity, btnCityidx) => {
        ...
        btnCity.addEventListener('keydown', (e) => {
            if (e.keyCode === 'Enter' || e.key === ' ') {
                console.log('(2-2) enter ok');
                titleChange(btnCities, btnCity);
                console.log('(3-2) title chg');
                // 시도 키워드 노출
                topParents.setAttribute('data-keyword', '5');
                console.log('(4-2) open');
                // 키워드 변경
                for (let i in keywordCol05Contents) {
                    console.log('(5-2) sigunguList');
                    if (btnCity.getAttribute('data-city') == i) {
                        mapContent.innerHTML = keywordCol05Contents[i];
                        console.log('(6-2) innerHTML');
                    }
                }
                // col5의 타이틀, 키워드에 tabindex부여
                mapContent.querySelectorAll('.keyword-box > div, li').forEach((el) => {
...
function issueTreeSigungu() {
    ...
    btnCities.forEach((btnCity, btnCityidx) => {
        // 클릭 시 시군구 지도 보임
        btnCity.addEventListener('click', () => {
            // console.log('(2) (2) (2) keyCode::' + e.keyCode);
                // console.log('(2) (2) (2)ENTERtargetID::' + e.target.id);
                console.log('(2222) enter ok');
                titleChange(btnCities, btnCity);
                console.log('(3333) title chg');
                sigunguMap.setAttribute('data-open', 'open');
                console.log('(4444) open');
                sigunguMap.setAttribute('data-inmap', btnCity.getAttribute('data-city'));
                for (let i in sigunguList) {
                    console.log('(5555) sigunguList');
                    if (sigunguMap.getAttribute('data-inmap') == i) {
                        sigunguLineup.innerHTML = sigunguList[i];
                        console.log('(6666) innerHTML');
                    }
                }
                // 첫번째 시군구로 포커스 이동
                setTimeout(() => {
                    console.log('(7777) focus go');
                    sigunguLineup.querySelector('li:first-child button').focus();
                }, 1);
                // 시군구 키워드 숨김
                console.log('(8888) hdd');
                topParents.setAttribute('data-keyword', '');
                console.log('(9999) remove');
                ChangeSigunguButton();
                console.log('(101010) zz');
            titleChange(btnCities, btnCity);
            console.log('(클릭시1) keyCode::' + btnCities);
            sigunguMap.setAttribute('data-open', 'open');
            console.log('(시군구지도2) keyCode::' + EventTarget.keyCode);
            sigunguMap.setAttribute('data-inmap', btnCity.getAttribute('data-city'));
            console.log('(시군구지도3) keyCode::' + Event.keyCode);
            for (let i in sigunguList) {
                console.log('(시군구지도4) keyCode::' + Event.keyCode);
                if (sigunguMap.getAttribute('data-inmap') == i) {
                    console.log('(시군구지도5) keyCode::' + Event.keyCode);
                    sigunguLineup.innerHTML = sigunguList[i];
                }
            }
            // 시군구 키워드 숨김
        ...
        btnCity.addEventListener('keydown', (e) => {
            console.log('(cmm) Key Pressed:', e.key);
        });
        btnCity.addEventListener('keyup', (e) => {
            console.log('(cmm) Keyup Pressed:', e.key);
        });
        btnCity.addEventListener('keypress', (e) => {
            console.log('(cmm) keypress Pressed:', e.key);
        });
        // btnCity.addEventListener('13', (e) => {
        //     console.log('(cmm) 13 Pressed:', e.key);
        // });
        // btnCity.addEventListener('32', (e) => {
        //     console.log('(cmm) 32 Pressed:', e.key);
        // });
        var idHtml = $("#cityBTN").html();
        console.log(idHtml);
        $("#cityBTN").keydown(function(e){
            console.log('keydownkeydownkeydownkeydownkeydownkeydownkeydownkeydownkeydown');
            if(e.key === 'Enter' || e.keyCode === 13){
            // if(e.keyCode == 13){
              // 호출할 함수나 기능 작성
              console.log('keyEnterkeyEnterkeyEnterkeyEnterkeyEnterkeyEnterkeyEnterkeyEnter');
             }
            if(e.key === 'Tab' || e.keyCode === 9){
            // if(e.keyCode == 13){
              // 호출할 함수나 기능 작성
              console.log('keyTabkeyTabkeyTabkeyTabkeyTabkeyTabkeyTabkeyTabkeyTabkeyTabkeyTabkeyTabkeyTabkeyTabkeyTab');
             }
        });
        ...
        btnCity.addEventListener('keydown', (e) => {
            ...
            console.log('(1) (1) (1) target:' + e.target);
            console.log('(1) (1) (1) targetID::' + e.target.id);
            console.log('(1) (1) (1) keyCode::' + e.keyCode);
            
            console.log('(1) btnCity?? ' + btnCity);
            console.log('(1) btnCity?? ' + btnCity.DOCUMENT_NODE);
